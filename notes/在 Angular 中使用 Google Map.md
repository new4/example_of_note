...
要在地图上显示出设备位置，自然而然的我们想到使用 marker 标记，在 `angular-google-maps` 中与 marker 相关的指令有两个：`ui-gmap-marker` 和 `ui-gmap-markers`。从命名上也可以看出来，前者显示的是单个的 marker，后者用于一次显示多个 marker。

根据我们的需求，一个服务器连着多台不同种类的设备，因为服务器只有一个，可以用 `ui-gmap-marker` 指令予以显示；其它设备有多台而且可以分成不同种类，那么可以按照类型划分设备，然后对同种类的设备使用 ui-gmap-markers 指令进行显示。
...
- options：指令的配置项，详细选项可以参考[这里](https://developers.google.com/maps/documentation/javascript/reference#MarkerOptions) ，需要注意的一个属性是 options.icon，我们可以利用这个属性来引入自己的 marker 标记图片
- events：指定在 marker 上发生的事件处理函数，关于 marker 标记的详细 events 可以看[这里](https://developers.google.com/maps/documentation/javascript/reference#Marker)

了解了 `ui-gmap-marker` 指令主要的属性，可以着手写在地图上显示服务器的代码了。
...
上述的属性参考的值同 `ui-gmap-marker` 指令中的参考值一致，但是有一点需要注意：像 coords, icon, options 等属性可以设置成指定的单个 marker 标记的对应属性。也就是说，models 数组中的每一个 marker 标记都含有 coords, icon, options 等属性，我们使用 `ui-gmap-markers` 指令的时候可以通过在 coords, icon, options 等属性外面加上单引号 '' 来指定当生成这些 marker 的时候是从对应 marker 自己内部同名属性中取值。

假设我们有两种类型的设备 deviceA 和 deviceB，对应的我们就可以使用两组标记（ 假定为 markersDeviceA 和 markersDeviceB ）表示它们，这里我们首先构造一组包含有设备基本信息的数据，同时用函数 createMarkers 负责两组标记的生成工作。另外，为了区分不同设备，在网上找了两个图标 deviceA.png 和 deviceB.png 来表示不同种类型的设备（侵删）。
...
在 `index.html` 中的 `ui-gmap-google-map` 指令内加上代码:

```html
<!-- device A markers -->
<ui-gmap-markers
	models="deviceAMarkers.markers"
	coords="'coords'"
	icon="'icon'"
...
目前我们已经成功的在地图上显示出了所有的设备，现在我们需要显示多一些信息：网络拓扑以及设备的状态。

设备与服务器之间的连接可以用简单的连线进行表示，设备被管理的状态可以用连线的颜色表示（红色表示未被管理，蓝色表示被管理）。

此处利用 `ui-gmap-polyline` 指令和 `ng-repeat` 指令进行绘制。

ui-gmap-polyline 指令的主要属性有：
- path：定义连线起始点和终止点的经纬度坐标
- stroke：可以定义连线的颜色、线宽和不透明度
- clickable：定义连线是否响应 click 事件
- draggable：定义连线是否可拖动
- editable：定义连线是否可编辑（改变形状之类的）
- geodesic：定义连线是否适应地球的曲率
- visible：显示或者隐藏连线
- fit：使地图边界和连线相适
- events：事件响应处理函数，关于 polyline 的所有 events 事件的详细内容可以看[这里](https://developers.google.com/maps/documentation/javascript/reference#Polyline)

在实现时的基本思路与我们生成设备的 marker 一致，我们也将连线按照设备 deviceA 和 deviceB 进行划分，用函数 createLines 负责两组连线的生成工作。
...
此处就用到 marker 的 events 选项了，events 属性中有一项是 drag，它指定了在 marker 发生拖动事件时的处理函数，我们可以在这个处理函数中隐藏连线；events 还有一个属性项是 dragend，可以用它指定拖动事件结束时的处理函数，我们可以在这个处理函数中设置新的连线的端点并将隐藏的连线显示出来。
...
<ui-gmap-markers
	...
	icon="'icon'"
	options="'options'"
	events="deviceAMarkers.events"></ui-gmap-markers>
...
<ui-gmap-markers
	...
	icon="'icon'"
	options="'options'"
	events="deviceBMarkers.events"></ui-gmap-markers>
```

此时的拖动行为就是正常的了，进一步了解 Marker events 的其它事件可以看[这里](https://developers.google.com/maps/documentation/javascript/reference#Marker)。
...
如果你对这个指令的部分内容不满意，想在代码中获得更多的自由，直接调用 [Google Maps APIs](https://developers.google.com/maps/documentation/javascript/3.exp/reference) 也是完全可以的，这时可以通过在 `ui-gmap-google-map` 指令 events 属性中监听 tilesloaded 事件来获取当前地图的实例，取得了当前的地图实例之后就可以利用它来使用诸如 Places 等库的功能，也就可以进一步拓展你的项目的功能了。
	...
	events: {
		tilesloaded: function(map) {
			$scope.$apply(function() {
				console.log( "在这里获得了当前地图的实例 map = ", map );
			});
		}
	}
};
